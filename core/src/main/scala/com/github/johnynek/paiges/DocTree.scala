...
object DocTree {
  ...
  private[paiges] def toDocTree(d: Doc): DocTree = {
    ...
    def fits(pos: Int, d: DocTree, minV: Int): Int = d.unfix match {
      ...
      case Right(Break(_)) #:: _ => pos min minV
      case Right(Str(s)) #:: tail =>
    ...
    def loop(pos: Int, lst: List[(Int, Doc)], bounds: Bounds): DocTree = lst match {
      ...
      case (i, Text(s)) :: z => docTree(Right(Str(s)) #:: cheat(pos + s.length, z, bounds).unfix)
      case (i, Line) :: z => docTree(Right(Break(i)) #:: cheat(i, z, bounds).unfix)
  ...
  private[paiges] def deunioned(d: DocTree): Stream[Doc] = {
    ...
    def loop(tree: DocTree, prefix: Doc): Stream[Doc] = tree.unfix match {
      ...
      case (Right(Str(t)) #:: tail) =>
        loop(docTree(tail), cat(prefix, Text(t)))
      case (Right(Break(n)) #:: tail) =>
  ...
  private def space2(n: Int) = Right(Str(" " * n)) // can memoize this
  ...
  private[paiges] def setDiff(a: DocTree, b: DocTree): Option[DocTree] =
    (a.unfix, b.unfix) match {
      ...
      case (Right(Break(nx)) #:: tailx, (right@Right(Break(ny))) #:: taily) if (nx == ny) =>
        setDiff(docTree(tailx), docTree(taily)).map { diff =>
          docTree(right #:: diff.unfix)
        }
      case ((left@Right(Str(s1))) #:: xtail, (right@Right(Str(s2))) #:: ytail) =>
  ...
  def isSubDoc(a: DocTree, b: DocTree): Boolean =
    (a.unfix, b.unfix) match {
      ...
      case (Right(Break(nx)) #:: tailx, Right(Break(ny)) #:: taily) =>
      ...
      case (Right(Break(_)) #:: _, _) => false // line comes after text (different from ascii!)
      case (_, Right(Break(_)) #:: _) => false
      case (Right(Str(s1)) #:: xtail, Right(Str(s2)) #:: ytail) =>
        if (s1.length == s2.length) {
  ...
  def compareTree(xs: DocTree, ys: DocTree): Int = {
    (xs.unfix, ys.unfix) match {
      ...
      case (Right(Break(_)) #:: _, _) => 1 // line comes after text (different from ascii!)
      case (_, Right(Break(_)) #:: _) => -1
      case (Right(Str(s1)) #:: xtail, Right(Str(s2)) #:: ytail) =>
        if (s1.length == s2.length) {
          val c = s1 compare s2
